...
class TicketAdminView(CommAdminView):
    def get(self, request):
        ...
        for time_code, times in preach_times_dict.items():
            ...
            preach_available_tickets[str(i)] = (times, r.llen('G-'+time_code+'-tickets'), r.get('preach_people-'+str(i)))
            i += 1
        i = 1
        for time_code, times in visit_times_dict.items():
            # visit_available_tickets[times] = r.llen('V-'+time_code+'-tickets')
            visit_available_tickets[str(i)] = (times, r.llen('V-'+time_code+'-tickets'))
            i += 1
        ...
        return render(request, 'ticket_info_lxy.html', context)  # 最后指定自定义的template模板，并返回context
...
class TicketControlView(CommAdminView):
    ...
    def post(self, request):
        r = redis.StrictRedis(decode_responses=True)
        preach_people1 = request.POST.get('preach_people1', '学长学姐')
        preach_people2 = request.POST.get('preach_people2', '学长学姐')
        r.set('preach_people-1', preach_people1)
        r.set('preach_people-2', preach_people2)
        all_ticket = r.keys('*-tickets')
        for key in all_ticket:
            r.delete(key)
        r.close()
